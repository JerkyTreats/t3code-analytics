...
function renderLineChart(targetId, rows, field, startId, endId, peakId) {
  ...
  target.innerHTML = `<svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="${gradientId}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#79f27d" stop-opacity="0.28"/><stop offset="1" stop-color="#79f27d" stop-opacity="0.01"/></linearGradient></defs><g class="grid">${grid}</g><polygon class="area" fill="url(#${gradientId})" points="${areaPoints}"/><polyline class="line" points="${points.join(" ")}"/></svg>`;
